...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('contact.show', $contact) }}">{{ $contact->name }}</a></li>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-default">
                ...
                <form action="{{ route('contact.update', $contact) }}" method="POST" class="form">
                    <div class="card-body">
                        {{-- Agar tidak 419 expired, ketika di simpan --}}
                        @csrf
                        {{-- Secara default form hanya support GET, POST --}}
                        {{-- Untuk mengubahnya menjadi PUT, perlu bantuan kode dibawah --}}
                        @method('PUT')
